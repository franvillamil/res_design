...
\small
\begin{enumerate}
  \item \textbf{Why design at all?} \asher{(today)}
  \vspace{6pt}
...
The last line is the one that matters, and it is the Challenger lesson restated: with only failures, the outcome column does not vary, so no comparison is available. Worth saying that explicitly, since it links the vocabulary back to the opening.}
...
This sets up the hotel booking example later in the session, where a badly chosen unit quietly changes what is being asked.}
...
\vspace{18pt}
\uncover<2->{A lot of what you will work with in this MA is \colorbox{yellow}{found data}}\\\vspace{15pt}
\uncover<2->{\small (Technically, when you use existing surveys or datasets that's also found data, but it is not the same as in typical CSS data, because it was collected with a research design in mind, and that's usually documented)}\\

\end{frame}
% ----------------------------------------------------
\note{One frame now, a full block next week, but plant the distinction here because the two examples coming up are both found data, and so is almost every project they will propose.
...
% ----------------------------------------------------
\begin{frame}
\frametitle{The problem of starting with the data first}
\centering

\vspace{10pt}
\begin{flushleft}
Two examples, both started the way most of yours will start: \textbf{with data already in hand} and \textbf{no question yet} (which shouldn't be the case, but...)
\end{flushleft}
...
\end{frame}
% ----------------------------------------------------
\note{The slide says only ``two examples''. Mention on the spot that they are real theses or course projects from this programme, without naming anyone. Saying they are real changes how they listen: these are not published papers by professors, they are the thing they themselves have to produce.
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Example 1: hotel bookings}
\centering
...
\begin{itemize}[<+->]
  ...
  \vspace{12pt}
  \item \textbf{Motivation:} we can use data to increase business
  \vspace{6pt}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Example 1: what can these data answer?}
\centering
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Example 2: music collaborations}
\centering
...
\vspace{8pt}
\begin{itemize}
  \item Spotify API data
  \item Build a turned a \textbf{network} of artist collaborations
  \item Variables: time, genre
\end{itemize}
...
% ----------------------------------------------------
\begin{frame}
\frametitle{Example 2: what can we ask here?}
\centering

\begin{itemize}[<+->]
  \item What is the unit of analysis? (before and after?)
  \item What questions can we answer here? General design approach?
  \item Problems of data origin (e.g are genres becoming more connected over time or catalogue issues? who decides how to tag artists?)
\end{itemize}
...
This is exactly the Google Flu problem, which is next week's paper. Say so, because it makes the reading feel like it was assigned for a reason.}
...
Both examples we just discussed entered this diagram in the middle (at ``Measure'') and had to work backwards to the question. That is normal, and the diagram does not show it.}
...
\vspace{12pt}
\begin{itemize}
  \item<2-> \textbf{Nothing to hand in, no mark.} But part of the session is a conversation about a paper you'll have to have read
\end{itemize}
...
State the consequence rather than threatening: if you have not read it, part of the session is a conversation you cannot join.
...
\begin{itemize}
  ...
  \vspace{6pt}
  \item<3-> \colorbox{yellow}{Groups are fixed by \textbf{Mon Oct 19}}, when Memo 5 is due. Whoever submits Memo 5 together presents together
\end{itemize}

\end{frame}
% ----------------------------------------------------
\note{35\% of the grade, and the essay group is the same group: 75\% of their mark comes out of this one project. Say so now.

\textbf{``The threat I can't solve''} is required and graded \textit{positively}. A design with no acknowledged limitation is either trivial or not understood. It removes the incentive to oversell. Say it twice, they will not believe you the first time.

\textbf{Group formation} needs no administration from you: whoever submits Memo 5 together on Oct 19 is a group, binding for the presentation and the essay. Five weeks to sort themselves out.

\textbf{Paired groups.} Each reviews one other, from the partner's Memo 5 (a week ahead). Written comments, handed in, graded on the group. Worth 10\% because reviewing an assigned document gets specific comments from everyone, while open feedback gets vague ones from the three loudest.}
...
\end{frame}
% ----------------------------------------------------
\note{The replication crisis, in one headline. Do not spend long, since we come back to it properly in week 5 with Breznau and the many-analysts study.
