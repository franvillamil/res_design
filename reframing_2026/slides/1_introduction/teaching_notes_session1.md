...
**A note on method throughout.** This is a room of forty in an evening slot. Open questions
collapse to the same three voices by about 19:30, so every discussion prompt in this deck should
be run as pose, think alone, pair, then take a few contributions, and you should call on pairs
rather than individuals. Where a question has a right answer, use a show of hands instead. This
is marked at the specific slides below, but it applies everywhere.
...
**The Socratic centre of the session, and the place where class size changes the method.**

With twelve students, silence is productive and you wait it out. **With forty it is not**: the
same three extroverts fill it, everyone else learns that they do not have to think, and you have
set that norm for five weeks.

So run every question here as **think-pair-share**: pose it, *sixty seconds of silence to think
alone*, two minutes in pairs, then take three or four contributions. It feels slower and it is
not, because the pair turn does the work that a long silence was supposed to do.

**Call on pairs, not individuals.** "What did you two land on?" is socially safe in a way that
naming someone is not, and it gets you answers from people who would never volunteer.
...
a genuinely useful reflex. The next three slides are practice.

**Run these three as a show of hands**, not as an open question. "Country, or country-year? Hands
up for country." Everyone participates, nobody has to speak, and you get an instant read on how
many are with you. At this class size it is the cheapest participation you will get all evening,
and it makes them commit before you reveal, which is the point.
...
**40 (`ua2`), and this one is in Spanish, so read it out.** *Median income per living unit, by
...
*(The image is a Spotify marketing banner, so decorative. If you find a real collaboration-network
figure, swap it in.)*
...
using them is normal, not an admission of failure. In a room this size, the students who come
...
news or anywhere: name the goal, say what data it would need, name the first thing you'd
doubt. Half a page.
